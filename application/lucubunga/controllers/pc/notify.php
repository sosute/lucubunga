	...
	public function index()
	{
		...
		else if ($result === 'VERIFIED')
		{
			...
			)
			{
				$message = ''.
					...
					'--- posted value from paypal ---'."\n".
					'  receiver_email: '.$post['receiver_email']."\n".
					'  mc_gross: '.$post['mc_gross']."\n".
					'  mc_currency: '.$post['mc_currency']."\n".
					'--- my config ---'."\n".
					'  receiver_email: '.$paypal_param_receiver_email."\n".
					'  mc_gross: '.$paypal_param_mc_gross."\n".
					'  mc_currency: '.$paypal_param_mc_currency."\n";
				trigger_error($message, E_USER_ERROR);
				exit;
			}
			...
			else
			{
				...
				if ($query->num_rows() > 0)
				{
					$message = ''.
						...
						'txn_id: '.$post['txn_id']."\n".
						'$post => '.var_export($post, TRUE);
					// DEBUG メール送信をしてしまうと逆に面倒なのでコメントアウト
					//trigger_error($message, E_USER_ERROR);
					exit;
				}
			}
		...
		if ($query->num_rows() > 0)
		{
			...
			else
			{
				...
				$this->email->from($this->config->item('my_mail_address'));
				$this->email->subject($this->load->view('pc/mail/paid/subject_for_admin', $app_values, TRUE));
				$this->email->message('[管理用] 本文は空にします。');
				if ( ! $this->email->send())
				{
					$err_message = ''.
