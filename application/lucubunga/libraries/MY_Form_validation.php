...
class MY_Form_validation extends CI_Form_validation {
    ...
    //protected $_error_prefix = '<div class="f_error_box">';
	...
	public function valid_passport_from_date()
	{
		...
		// 今日の日付より前に発行されているかチェック
		if ($today < $passport_from_date)
		{
			return FALSE;
		}
		// 誕生日より後に発行されているかチェック
	...
	public function passport_number($str)
	{
		...
		{
            if (
                ! isset($_POST['no_check_passport']) ||
                $_POST['no_check_passport'] !== 'true'
            )
            {
                if (preg_match('/^(M|T|R)[A-Z][0-9]{7}$/', $str) !== 1)
                {
                    return FALSE;
                }
            }
			else // 訂正旅券や公用旅券などお客様側でノーチェックを希望した場合は先頭アルファベット2ケタを検証しない
			{
                if (preg_match('/^[A-Z]{2}[0-9]{7}$/', $str) !== 1)
                {
                    return FALSE;
                }
			}
	...
	public function valid_passport_to_date()
	{
		...
		if ($_POST['country_national'] === 'JP')
		{
            // 公用旅券の場合及びユーザにてチェックOFFで送信された場合（訂正旅券など）は、妥当性を検証しない
            if (
                (preg_match('/^(MB|mb|R|r)/', $_POST['passport_number']) === 1) ||
                (isset($_POST['no_check_passport']) && $_POST['no_check_passport'] === 'true')
            )
			{
				return TRUE;
			}
